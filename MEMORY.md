...
| Item (§51) | Status |
|---|---|
| Package installs globally | ✓ `0.1.1`–`0.5.0` published to npm (user runs the 2FA-gated publish manually; latest = 0.5.0, tags v0.1.1–v0.5.0 pushed to GitHub). All verified from the registry: installed into clean temp prefixes → binaries incl. `glm-fast` work, `glm-worker` returned `REGISTRY_OK`/`V02_OK`; 0.3.0's `delegate` ran a real GLM worker returning `REGISTRY_V03_OK` with worktree+branch kept, and the ERROR [31]/ERROR [2] paths fired correctly from the registry shims; 0.4.0's `benchmark` ran a real GLM run returning PASS (50.2s / 7 turns) with the report saved, dry-run + INVALID_ARGS guards correct from the registry shims; 0.5.0's `usage` rendered the real quota from the monitor endpoint (text + --json) from the registry shims; machine's own global install tracks the registry (now 0.5.0). Fresh-machine install (§50) remains blocked on hardware |
